...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
	...
	private static Year getYear(int y) {
		...
		String yy = String.valueOf(y).toString();
	...
	static class Month {
		...
		private Day createDay(int d) {
			...
			el.addAttribute(
				new Attribute("date", new CalendarDate(d, getValue(), Integer.valueOf(((Element) mElement.getParent()).getAttribute("year").getValue())).toString()));
