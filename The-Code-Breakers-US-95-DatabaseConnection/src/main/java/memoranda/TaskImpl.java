...
public class TaskImpl implements Task, Comparable {
	...
	public CalendarDate getCustomDueDate(String classId, String studentId) {
    Elements overrides = _element.getChildElements("customDueDate");
    for (int i = 0; i < overrides.size(); i++) {
        Element e = overrides.get(i);
        String cId = e.getAttributeValue("classId");
        String sId = e.getAttributeValue("studentId");

        if ((classId == null || classId.equals(cId)) &&
            (studentId == null || studentId.equals(sId))) {
            return new CalendarDate(e.getValue());
        }
    }
    return null;
}

	public void setCustomDueDate(String classId, String studentId, CalendarDate date) {
    Elements overrides = _element.getChildElements("customDueDate");
    for (int i = 0; i < overrides.size(); i++) {
        Element e = overrides.get(i);
        if (classId.equals(e.getAttributeValue("classId")) &&
            ((studentId == null && e.getAttribute("studentId") == null) ||
             (studentId != null && studentId.equals(e.getAttributeValue("studentId"))))) {
            _element.removeChild(e);
            break;
        }
    }

    Element override = new Element("customDueDate");
    override.addAttribute(new Attribute("classId", classId));
    if (studentId != null)
        override.addAttribute(new Attribute("studentId", studentId));
    override.appendChild(date.toString());
    _element.appendChild(override);
}

	public CalendarDate getEndDate(String classId, String studentId) {
    CalendarDate custom = getCustomDueDate(classId, studentId);
    if (custom != null)
        return custom;
    return getEndDate(); 
}
