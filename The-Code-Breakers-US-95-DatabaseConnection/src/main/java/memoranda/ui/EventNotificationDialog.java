...
public class EventNotificationDialog extends JFrame {
  ...
  void jbInit() throws Exception {
   ...
   // this.getContentPane().setBackground(new Color(251, 197, 63));
    ...
    border3 = BorderFactory.createCompoundBorder(BorderFactory.createEtchedBorder(Color.black,new Color(255, 255, 255)),BorderFactory.createEmptyBorder(0,30,0,30));
  ...
  private void playSoundNotification() {
		...
		try {
			...
			//clip.play();
			AudioInputStream audioInputStrearm = AudioSystem.getAudioInputStream(new File(url.toString()).getAbsoluteFile());
			Clip clip = AudioSystem.getClip();
			clip.open(audioInputStrearm);
			clip.start();
