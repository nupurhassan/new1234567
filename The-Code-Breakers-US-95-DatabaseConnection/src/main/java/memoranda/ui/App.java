...
public class App {
	...
	public App(boolean fullmode, AuthenticationService authenticationService) {
		super();
		if (fullmode)
		...
		as = authenticationService; //Added for changing login/password while using system.
	...
	private void showSplash() {
		...
		JLabel l = new JLabel();
		l.setIcon(originalIcon); // Set the original icon without scaling

		splash.getContentPane().add(l);
		splash.setSize(originalIcon.getIconWidth(), originalIcon.getIconHeight()); // Set size to match the image

		// Center the splash screen on the screen
		Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
		splash.setLocation(
				(screenSize.width - originalIcon.getIconWidth()) / 2,
				(screenSize.height - originalIcon.getIconHeight()) / 2
		);
